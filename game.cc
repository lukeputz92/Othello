...
namespace main_savitch_14
{
   ...
   // The commenting you see below sets this up for Phase One	
    ...
    int game::eval_with_lookahead(int look_ahead =0, int beat_this =0)
    ...
    // moved. 
    {
		...
		int value=0;             // Value of a board position after opponent moves
    	int best_value=0;        // Evaluation of best opponent move
    	game* future;          // Pointer to a future version of this game
	
        // Base case:
	if (look_ahead == 0 || is_game_over( ) == true)
	{
	    if (last_mover( ) == COMPUTER)
	            return evaluate( );
	    else
